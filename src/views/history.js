    ...
    <div class="exercise-card list-item animate-fade-in-up stagger-${Math.min(i + 1, 8)}" data-workout-id="${w.id}">
      <div class="exercise-card-header" data-view-id="${w.id}" style="cursor:pointer;">
        ...
        <div class="flex gap-8" style="align-items:center;">
          ...
          `)}
          <button class="btn-icon" data-delete-workout="${w.id}" style="width:32px;height:32px;flex-shrink:0;" title="Delete workout">
  ...
  const renderDetail = () => {
            ...
            ${(workout.exercises || []).map((e, i) => html`
              <div class="glass-sm list-item animate-fade-in-up stagger-${Math.min(i + 1, 8)}" style="padding:14px 16px;">
                ...
                <div class="exercise-fields">
                  <div class="input-row">
                    <input class="input" type="number" inputmode="decimal" placeholder="Weight" data-edit-field="${e.name}" data-field="weight" value="${e.weight || ''}" ${workout._editing ? '' : 'disabled'} style="${workout._editing ? '' : 'opacity:0.6;'}"/>
                    <input class="input" type="number" inputmode="numeric" placeholder="Sets" data-edit-field="${e.name}" data-field="sets" value="${e.sets || ''}" ${workout._editing ? '' : 'disabled'} style="${workout._editing ? '' : 'opacity:0.6;'}"/>
                    <input class="input" type="number" inputmode="numeric" placeholder="Reps" data-edit-field="${e.name}" data-field="reps" value="${e.reps || ''}" ${workout._editing ? '' : 'disabled'} style="${workout._editing ? '' : 'opacity:0.6;'}"/>
                  </div>
                  <input class="input mt-8" type="text" placeholder="Notes" data-edit-field="${e.name}" data-field="notes" value="${e.notes || ''}" ${workout._editing ? '' : 'disabled'} style="${workout._editing ? '' : 'opacity:0.6;'}" />
                </div>
              </div>
            `).join('')}
          </div>
    ...
    document.getElementById('save-edit-btn')?.addEventListener('click', async () => {
      ...
      for (const ex of (workout.exercises || [])) {
        const weightEl = container.querySelector(`[data-edit-field="${ex.name}"][data-field="weight"]`);
        const setsEl = container.querySelector(`[data-edit-field="${ex.name}"][data-field="sets"]`);
        const repsEl = container.querySelector(`[data-edit-field="${ex.name}"][data-field="reps"]`);
        const notesEl = container.querySelector(`[data-edit-field="${ex.name}"][data-field="notes"]`);
        if (weightEl) ex.weight = weightEl.value;
        if (setsEl) ex.sets = setsEl.value;
        if (repsEl) ex.reps = repsEl.value;
